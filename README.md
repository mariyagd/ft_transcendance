- Télécharger le projet
    ...
    https://github.com/mariyagd/trans-test
...
- Endpoint: Register  (l’enregistrement d’un nouvel user)
    ...
    - Validateurs: auth conteneur exécute les validateurs suivants:
        ...
        - `email` : Required. Email address. (Vérifie s’il y a `@` et `.` dans l’adresse e-mail)
            ...
            *Pour l’instant on peut utiliser `localhost` comme domain de l’adresse email, mais je dois voir comment le supprimer. P.ex. `mariya@localhost` est validé.*
    ...
    - Valeurs retournées
        ...
        - `is_active` : Boolean.
            ...
            - *En gros ici ce qu’on peut faire, au lieu de supprimer un compte utilisateur avec DELETE, on peut mettre is_active=false et le garder dans la base de données pour permettre que l’utilisateur se reconnecte en restaurant toute l’historique des ses activités. Donc c’est encore à réfléchir.*
